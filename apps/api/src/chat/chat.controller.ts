...
export class ChatController {
    ...
    async sendMessage(
        ...
        @Body() data: { content: string, storeId: string, orderId?: string, recipientId?: string }
    ) {
    ...
    async getMessages(
        ...
        if (otherUserId) {
            return this.chatService.getUserMessagesForStore(userId, storeId, otherUserId);
        }
        return this.chatService.getMessages(userId, storeId);
    }

    @Get('conversations')
    async getConversations(@Query('storeId') storeId: string) {
