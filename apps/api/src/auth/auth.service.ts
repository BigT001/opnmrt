...
export class AuthService {
    ...
    async sendOtp(email: string, phone?: string) {
        ...
        try {
            ...
            if (error) {
                console.warn(`⚠️ [Resend Restricted] Email to ${email} failed. Use the OTP from console: ${otp}`);
    ...
    async register(input: RegisterInput & { subdomain?: string; storeName?: string }) {
        try {
            ...
            if (!isVerified) {
                throw new UnauthorizedException('Email/Phone not verified. Please verify OTP first.');
            }
        ...
        try {
            ...
            if (user.role === 'BUYER' && input.subdomain) {
                ...
                if (!targetStore || user.storeId !== targetStore.id) {
                    console.log(`Login restricted: Buyer ${input.email} does not belong to store ${input.subdomain}`);
                    throw new UnauthorizedException('This account is not registered with this store');
                }
            }
