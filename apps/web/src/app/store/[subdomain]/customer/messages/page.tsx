    ...
    return (
        <div className="flex flex-col h-[700px] space-y-6">
            <div className="shrink-0">
                ...
                <p className="text-slate-500 mt-1">Direct support from {storeInfo?.name || 'the seller'}</p>
            </div>
            ...
            <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col overflow-hidden relative">
                ...
                <div className="p-6 border-b border-slate-50 flex items-center gap-4 bg-slate-50/50">
                    ...
                    <div>
                        <h3 className="font-black text-slate-900 text-sm">{storeInfo?.name}</h3>
                ...
                >
                    {messages.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center p-10">
                            ...
                            <h4 className="text-sm font-black text-slate-900">No messages yet</h4>
                            ...
                            return (
                                ...
                                >
                                    <div className={`flex items-end gap-3 max-w-[80%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold shrink-0 shadow-sm ${isMe ? 'bg-slate-100 text-slate-600' : 'bg-slate-900 text-white'
                                            }`}>
                ...
                {/* Message Input */}
                <form onSubmit={handleSend} className="p-6 bg-slate-50/50 border-t border-slate-50">
                    <div className="relative">
                        <input
                            ...
                            className="w-full h-14 pl-6 pr-16 bg-white border border-slate-100 rounded-2xl text-xs font-bold outline-none focus:ring-4 focus:ring-slate-900/5 focus:border-slate-200 transition-all text-slate-900 shadow-sm"
                        />
                        <button
                            type="submit"
                            disabled={sending || !newMessage.trim()}
                            className="absolute right-2 top-2 w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center hover:bg-black transition-all disabled:opacity-50 shadow-lg shadow-slate-900/20"
                        >
