        ...
        <div className="min-h-screen bg-background text-foreground transition-colors duration-500 overflow-x-hidden relative">
            ...
            <main className="relative pt-32 pb-40">
                ...
                <section className="max-w-4xl mx-auto px-6 mt-40 text-center">
                    <div className="glass-panel p-12 rounded-[3rem] border border-border">
                        <h3 className="text-2xl font-black text-foreground mb-6 uppercase tracking-tighter italic">The OPNMRT Guarantee</h3>
                        <p className="text-muted-foreground font-bold uppercase tracking-widest text-[10px] mb-8">Full Sovereignty • No Hidden Fees • Bank-Grade Security</p>
...
function PricingCard({ tier, price, desc, features, icon, recommended }: any) {
    return (
        <motion.div
            whileHover={{ y: -10 }}
            className={`glass-panel p-10 rounded-[4rem] border ${recommended ? 'border-emerald-500/30' : 'border-border'} relative overflow-hidden flex flex-col`}
        >
            {recommended && (
            ...
            <div className="flex items-center space-x-3 mb-8">
                <div className={`p-3 rounded-2xl bg-muted ${recommended ? 'text-emerald-400 border border-emerald-500/20' : 'text-muted-foreground'}`}>
                    {icon}
                </div>
                <h3 className="text-2xl font-black text-foreground uppercase tracking-tighter">{tier}</h3>
            </div>

            <div className="mb-8">
                <div className="flex items-baseline space-x-1">
                    <span className="text-5xl font-black text-foreground tracking-tighter">{price === 'Custom' ? price : `$${price}`}</span>
                    {price !== 'Custom' && <span className="text-muted-foreground font-bold uppercase tracking-widest text-[10px]">/Month</span>}
                </div>
                <p className="mt-4 text-sm text-muted-foreground font-medium italic">{desc}</p>
            </div>

            <div className="space-y-4 mb-12 flex-1">
                {features.map((feature: string, i: number) => (
                    <div key={i} className="flex items-center space-x-3 group">
                        <div className={`w-5 h-5 rounded-lg flex items-center justify-center ${recommended ? 'bg-emerald-500/10 text-emerald-400' : 'bg-muted text-muted-foreground'}`}>
                            <Check size={12} />
                        </div>
                        <span className="text-sm font-bold text-muted-foreground group-hover:text-foreground transition-colors tracking-tight">{feature}</span>
                    </div>
                ))}
            </div>
...
function GuaranteeItem({ icon, text }: any) {
    return (
        <div className="flex items-center justify-center space-x-2 text-muted-foreground font-bold uppercase tracking-widest text-[10px]">
